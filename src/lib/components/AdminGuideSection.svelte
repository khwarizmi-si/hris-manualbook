...
>
	...
	<div class="hidden sm:block absolute top-20 right-0 w-80 h-80 rounded-full bg-brand-orange/5 blur-3xl pointer-events-none"></div>
	<div class="hidden sm:block absolute bottom-10 left-0 w-72 h-72 rounded-full bg-brand-teal/5 blur-3xl pointer-events-none"></div>

	<div class="max-w-6xl mx-auto relative z-10">
		<!-- Heading -->
		<div class="flex flex-col items-center text-center mb-12 sm:mb-16 section-header" class:in-view={isVisible}>
			<div class="inline-flex items-center gap-2 bg-orange-50 text-orange-800 text-[10px] sm:text-xs font-bold px-3 sm:px-4 py-1.5 sm:py-2 rounded-full mb-4 sm:mb-5 tracking-wide uppercase">
				{tr.badge}
			</div>
			<h2 class="text-2xl sm:text-3xl md:text-5xl font-bold text-gray-900 mb-3 sm:mb-4 text-balance">
				{tr.heading_plain}<span class="text-brand-orange">{tr.heading_accent}</span>
			</h2>
		...
		>
			<div class="grid lg:grid-cols-[1fr_1.45fr]">
				<div class="p-4 sm:p-6 lg:p-8 flex flex-col justify-center">
					<div class="inline-flex items-center gap-2 bg-orange-50 text-orange-800 text-[10px] sm:text-xs font-bold px-3 py-1.5 rounded-full mb-3 sm:mb-4 uppercase tracking-wide w-fit">
						<PlayCircle size={14} />
		...
		<div class="space-y-2.5 sm:space-y-3 max-w-4xl mx-auto">
			{#each guides as guide, i}
				<div
					...
					{openCard === guide.id
						? 'border-brand-orange/30 sm:border-brand-orange/40 shadow-lg shadow-orange-100/30'
						: 'border-gray-100 hover:border-brand-orange/20 hover:shadow-md shadow-sm shadow-gray-100/50'}"
					class:in-view={isVisible}
				...
				>
					...
					<button
						...
						id="accordion-header-admin-{guide.id}"
					...
					>
						<div class="flex items-center gap-2.5 sm:gap-4 min-w-0">
							<div class="flex-shrink-0 w-9 h-9 sm:w-11 sm:h-11 rounded-lg sm:rounded-xl flex items-center justify-center transition-all duration-300
								{openCard === guide.id
									? 'bg-brand-orange shadow-md shadow-orange-200/60'
									: 'bg-orange-50'}">
								<guide.icon size={18} class="sm:hidden {openCard === guide.id ? 'text-white' : 'text-brand-orange'}" />
								<guide.icon size={20} class="hidden sm:block {openCard === guide.id ? 'text-white' : 'text-brand-orange'}" />
							</div>
							<div class="min-w-0">
								<div class="text-[10px] sm:text-xs text-orange-800 font-bold mb-0.5 tracking-wide">
									{tr.section} {guide.id}
								</div>
						...
						<div class="flex-shrink-0 ml-2 sm:ml-4 w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center transition-all duration-300 ease-out
							{openCard === guide.id ? 'bg-brand-orange text-white rotate-180' : 'bg-gray-100 text-gray-400'}">
									...
									{#each guide.items as item}
										<div>
											<div class="flex items-center gap-2 mb-2 sm:mb-3">
												<ChevronRight size={14} class="text-brand-orange" />
												<h4 class="text-xs sm:text-sm font-bold text-gray-800">{item.sub}</h4>
											</div>
											<div class="border-l-2 border-brand-orange/20 pl-3 sm:pl-5">
												<ol class="space-y-2.5 sm:space-y-3">
													{#each item.steps as step, idx}
														<li class="flex gap-2.5 sm:gap-3 items-start">
															<span class="flex-shrink-0 w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-brand-orange text-white text-[10px] sm:text-xs font-bold flex items-center justify-center mt-0.5 shadow-sm">
																{idx + 1}
															</span>
										...
										{#if guide.videoSrc}
											<div class="rounded-xl overflow-hidden border border-gray-200 shadow-md bg-white">
												<div class="bg-gray-50 border-b border-gray-200 px-3 py-2 flex items-center gap-2.5">
													<PlayCircle size={14} class="text-brand-orange flex-shrink-0" />
