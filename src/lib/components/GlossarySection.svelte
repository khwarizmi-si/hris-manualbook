...
>
	<!-- Blobs -->
	<div class="absolute top-0 right-10 w-80 h-80 rounded-full bg-brand-teal/6 blur-3xl pointer-events-none"></div>
	<div class="absolute bottom-0 left-0 w-72 h-72 rounded-full bg-brand-orange/5 blur-3xl pointer-events-none"></div>
	...
	<div class="max-w-7xl mx-auto relative z-10">
		...
		<div class="flex flex-col items-center text-center mb-16 section-header" class:in-view={isVisible}>
			<h2 class="text-3xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-4 text-balance">
				<span class="text-brand-teal">{tr.heading_accent}</span>
			</h2>
				...
				>
					<div class="flex items-start gap-4">
						<div class="flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 group-hover:scale-110
							{item.color === 'teal' ? 'bg-brand-teal/10' : 'bg-brand-orange/10'}">
							<item.icon size={18} class={item.color === 'teal' ? 'text-brand-teal' : 'text-brand-orange'} />
						</div>
						<div class="flex-1 min-w-0">
							<div class="text-sm font-bold text-gray-900 mb-1 flex items-center gap-2">
								{item.term}
								<span class="w-1.5 h-1.5 rounded-full flex-shrink-0
									{item.color === 'teal' ? 'bg-brand-teal' : 'bg-brand-orange'}"></span>
							</div>
